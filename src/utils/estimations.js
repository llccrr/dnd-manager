export const estimateAc = (target, touchScore, value) => {
    ...
    if (value > 0 || value === 0) {
        return {
            acMax: target.acMax ? Math.min(target.acMax, parseInt(touchScore)) : parseInt(touchScore),
        };
    } else {
        return {
...
export const estimateHps = (target, hpState, damageReceived) => {
    ...
    if (target.hpState === hpState) {
        if (hpState === 'green') {
            hpMin = Math.max(3 * damageReceived + 1, targetHpMin);
        }
        if (hpState === 'orange') {
            hpMin = Math.max((3 / 2) * damageReceived + 1, targetHpMin);
        }
        if (hpState === 'red') {
            hpMin = Math.max(1 * damageReceived + 1, targetHpMin);
        }
    } else {
        if (hpState === 'dead') {
            ...
            hpMin = Math.max(damageReceived, targetHpMin);

            if (hpState === 'orange') {
                hpMax = Math.min(3 * damageReceived, targetHpMax);
            }

            if (hpState === 'red') {
                hpMax = Math.min((3 / 2) * damageReceived, targetHpMax);
            }
        }
    }
